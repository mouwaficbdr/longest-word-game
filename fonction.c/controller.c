...
void AfficherGagnantTour(){
    ...
    gotoxy(((largeurR)- strlen("GAGNANT DU TOUR   "))/2 + xR, yR + 4);printf("GAGNANT DU TOUR %d", Partie.tourJoues);
    ...
    if(Joueur1.score[Partie.tourJoues - 1] > Joueur2.score[Partie.tourJoues - 1]){
        ...
        gotoxy(xR + 6, yR + 8);printf("%s vous etes le gagnant de ce tour", Joueur1.nom);
    ...
    //Si le score du joueur 2 pour le tour actuel est plus élevé par rapport à celui du joueur 1, il est le gagnant
        ...
        gotoxy(xR + 6, yR + 8);printf("%s vous etes le gagnant de ce tour", Joueur2.nom);
...
void AfficherGagnantPartie(){
    ...
    //Si les scores sont égaux, il s'agit d'un match nul
    }else{
        gotoxy(xR + 6, yR + 6);printf("Match nul !!!");
...
void DemarrerPartieSolo(char Joueur1name[], NiveauDifficulteIA niveauIA, int tourActuel, int totalTours, int numCommencer) {
    ...
    if (numCommencer == 1) {
        ...
        EffacerZone(EntryFieldX2, EntryFieldY2 - 1, strlen("L'IA reflechit..."), 1);
...
void lancerJeuSolo() {
    ...
    if (!vientDeCharger) {
        Effacer();
        char TexteNiveau[] = "Choisissez le niveau de difficulte de l'IA:";
